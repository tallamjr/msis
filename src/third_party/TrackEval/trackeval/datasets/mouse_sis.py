...
class MouseSIS(_BaseDataset):
    """Dataset class for YouTubeVIS tracking"""
    @staticmethod
    def get_default_dataset_config():
        """Default class config values"""
        code_path = Path(sys.argv[0]).parent.parent

        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/MouseSIS'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'output'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval
            'CLASSES_TO_EVAL': None,  # Classes to eval (if None, all classes)
            'SPLIT_TO_EVAL': 'test',  # Valid: TODO: place valids
    ...
    def __init__(self, config=None):
        ...
        self.gt_file = Path(self.config['GT_FOLDER']) / f'{self.config["SPLIT_TO_EVAL"]}_annotations.json'
        ...
        if not self.gt_file.exists():
            raise TrackEvalException("GT file not found: " + str(self.gt_file))
        ...
        for tracker in self.tracker_list:
            tracker_dir_path = Path(self.tracker_fol) / tracker
            #tr_dir_files = [file for file in os.listdir(tracker_dir_path) if file.endswith('.json')]
            tr_dir_files = [file.name for file in tracker_dir_path.glob('*.json')]
        ...
        if len(self.tracker_list) == 1:
            config_path = Path(self.tracker_fol) / self.tracker_list[0] / 'config.yaml'
            with open(config_path) as f:
                config = yaml.safe_load(f)
            sequence_ids = config['common']['sequence_ids']
        else:
            sequence_ids = None

        self.seq_list, self.seq_name_to_seq_id, self.seq_lengths = [], {}, {}

        for vid in self.gt_data['videos']:
            if sequence_ids is None or int(vid['id']) in sequence_ids:
                self.seq_list.append(vid['id'])
                self.seq_name_to_seq_id[vid['id']] = vid['id']
                self.seq_lengths[vid['id']] = vid["length"] - 1
